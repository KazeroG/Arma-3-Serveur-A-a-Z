...
*/
private["_uid","_name","_side","_money","_bank","_misc","_query","_result","_array","_firstjoin_string"];
...
switch (_side) do
{
	case west:
	{
		...
		_query = format["INSERT INTO players (playerid, name, cash, bankacc, cop_gear, aliases, cop_licenses, civ_licenses, civ_gear, inventory, anticheat_mot, perms) VALUES('%1', '%2', '%3', '%4', '%5', '%6', '""[]""', '""[]""', '""[]""', '""[]""', 'Joined', '""[]""')",_uid,_name,_money,_bank,_misc,_alias]; //##43
	};
	
	case civilian:
	{
		if(typeName _misc == "BOOL") then {_misc = [_misc] call DB_fnc_bool;};
		_query = format["INSERT INTO players (playerid, name, cash, bankacc, arrested, aliases, civ_gear, cop_licenses, civ_licenses, cop_gear, inventory, anticheat_mot, perms) VALUES('%1', '%2', '%3', '%4', '%5', '%6','""[]""', '""[]""', '""[]""', '""[]""', '""[]""', 'Joined', '""[]""')",
		_uid, //1
		_name, //2 
		_money,//3
		_bank, //4
