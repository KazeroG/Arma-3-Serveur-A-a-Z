...
life_cur_task setSimpleTaskDescription [format["Du sollst dieses Päckchen zu %1 bringen.",toUpper _dp],"Delivery Job",""];
life_cur_task setTaskState "Assigned";
player setCurrentTask life_cur_task;

["DeliveryAssigned",[format["Bringe dieses Päckchen zu %1",toUpper _dp]]] call bis_fnc_showNotification;
...
[] spawn
{
	...
	if(!alive player) then
	{
		...
		["DeliveryFailed",["Du konntest das Päckchen nicht überbringen."]] call BIS_fnc_showNotification;
