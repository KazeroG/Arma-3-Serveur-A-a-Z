...
if(count _vehicleInfo == 0) exitWith {hint "Fahrzeug ist leer"};
...
if(_value > 0) then
{
	[[0,format["Ein Fahrzeug wurde durchsucht und hat illegale Items im Wert von $%1 gelagert.",[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	life_atmcash = life_atmcash + _value;
	_vehicle setVariable["Trunk",[],true];
}
	else
{
	hint "Nichts illegales im Fahrzeug.";
};