...
*/
private["_display","_item_list","_gear_list","_shop_data","_name","_price","_isillegal","_marketprice"];
...
////////Shop item list
{
	...
	if(_index != -1) then
	{
		...
		if(_x == "illegalmoney") then
		{
			_item_list lbAdd format["$%1 (Illegal)",[_price] call life_fnc_numberText];
		}
		else
		{
 ...
 */
{
	...
	if(_val > 0) then
	{
		_isillegal = _x in life_illegal_item_shortnames; //if illegal
	
		if(_isillegal) then
		{
			_gear_list lbAdd format["%1x %2 (Illegal)",_val,_name];
		}
		else
		{
